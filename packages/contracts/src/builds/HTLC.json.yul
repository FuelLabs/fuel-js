  ...
  result := div(mload(position), exp(2, sub(256, mul(length, 8))))
    ...
    function transfer(amount, token, owner) {
      ...
      case 0x2b660100 {
        ...
        require(eq(operator, caller()), 0x07)
        ...
        // Token is an ERC20.
        default {
          // get ERC20 balance of from token.
          mstore(0, 0x70a08231) mstore(add(0,32), address())
          require(call(gas(), token, 0, 28, 36, 0, 32), 0x09)
        ...
        // Notate new hash in storage and remap total locked for token.
      ...
      case 0xa49f2c25 {
        ...
        let correct := eq(sha256(160), mload(64))
        ...
        require(sload(mappingKey(1, hash)), 0x0b)
        ...
        if iszero(expired) {
          ...
          default {
            mstore(0, 0xa9059cbb) mstore(add(0,32), owner) mstore(add(0,64), amount)
            require(call(gas(), token, 0, 28, 68, 0, 0), 0x0c)
          }
        }
