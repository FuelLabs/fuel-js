...
SimulateDown.prototype._open = function (options, callback) {
  const self = this;
  openIfNew(self.store, options, err => !err
    ? openIfNew(self.cache, options, callback)
    : callback(err));
}

SimulateDown.prototype._close = function (callback) {
  const self = this;
  closeIfOpen(self.store, err => !err
    ? closeIfOpen(self.cache, callback)
    : callback(err));
}
...
SimulateDown.prototype._put = function (key, value, options, callback) {
  ...
  self.cache._put(key, value, options, (putError, result) => {
    ...
    self.cache._del(deleteKey(key), options, (deleteError, deleteResult) => {
      callback(putError, result);
    });
  });
}
...
SimulateDown.prototype._get = function (key = {}, options = {}, callback) {
  ...
  self.cache.get(key, options, (cacheError, result) => {
    if (cacheError) {
      return self.cache.get(deleteKey(key), options, (deleteError, cacheResult) => {
        if (deleteError) {
...
SimulateDown.prototype._del = function (key = {}, options = {}, callback) {
  const self = this;
  self.cache._del(key, options, (notCacheDelete, result) => {
  ...
  this.cache._batch(array.reduce((acc = [], entry = {}) => {
    ...
    if (entry.type === 'del') {
      return acc.concat([ entry, {
          type: 'put',
          key: deleteKey(entry.key),
          value: encoding.valueEncoding.encode(key),
        }]);
    }
  }, []), options, callback);
};
